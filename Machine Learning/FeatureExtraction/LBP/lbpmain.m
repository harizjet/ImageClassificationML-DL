...
for n=1:length(filename)
     ...
     K=imresize(X, [224 224]);        	 %  resize the image, also note that image size also could affect feature extraction
     I=rgb2gray(K);    		   	 % convert the colour image into gray scale
     features = extractLBPFeatures(I);	%  extract LBP feature
     HH(n + current_pos,:) = features;		 % store LBP feature data into column matrix
