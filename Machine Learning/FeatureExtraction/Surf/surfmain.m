...
for n=1:length(filename)    
     ...
     features = detectSURFFeatures(I);	%  extract SURF feature
     [tempfeatures, valid_points] = extractFeatures(I, features);
     
     valid_points = valid_points.selectStrongest(50); % only select the top 50 points
     
     tempsingle = [];
     for i=1:height(table(valid_points.Location))    % loops over every row in the table of the valid point
         temppoint = valid_points.Location(i, 1:2);    % get the (x, y) position of the valid corner
         euclid_dist = sqrt((temppoint(1)-0)^2 + (temppoint(2)-0)^2);   % calculate the euclidean distance between the valid point and 0,0
         tempsingle(i) = euclid_dist;   % append the euclidean distance into an array
     end
     
     HH(n + current_pos,1:length(tempsingle)) = tempsingle;     % store SURF feature data into column matrix
...
for n=1:length(filename1)
     ...
     K=imresize(X, [100 100]);        	 %  resize an image
     features = detectSURFFeatures(I);	%  extract SURF feature
     [tempfeatures, valid_points] = extractFeatures(I, features);
     
     valid_points = valid_points.selectStrongest(10); % only select the top 10 points
